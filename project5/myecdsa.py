...
class CryptECDSA(object):
    ...
    def verify(self, r,s, data):
        e = int(data.hex(), 16)
        w = pow(s, int(self.ecc_table['n'], 16) - 2, int(self.ecc_table['n'], 16))
        P1 = self._kg((e * w) % int(self.ecc_table['n'], 16), self.ecc_table['g'])
        P2 = self._kg((r * w) % int(self.ecc_table['n'], 16), self.public_key)
        if P1 == P2:
            P1 = '%s%s' % (P1, 1)
        ...
        return (r == (x % int(self.ecc_table['n'], 16)))
    ...
    def sign(self, data, K):  # 签名函数, data消息的hash，private_key私钥，K随机数，均为16进制字符串
        ...
        R = int(P1[0:self.para_len], 16)
        
        if R == 0:
            return None
        k_1 = pow(k, int(self.ecc_table['n'], 16) - 2, int(self.ecc_table['n'], 16))
        S = (k_1 * (e + d * R)) % int(self.ecc_table['n'], 16)
        return R,S

if __name__ =="__main__":
    private_key = '00B9AB0B828FF68872F21A837FC303668428DEA11DCD1B24429D0C99E24EED83D5'
    public_key = 'B9C9A6E04E9C91F7BA880429273747D7EF5DDEB0BB2FF6317EB00BEF331A83081A6994B8993F3F5D6EADDDB81872266C87C018FB4162F5AF347B483E24620207'
    ecdsa_crypt = CryptECDSA(public_key=public_key, private_key=private_key)
    data = b"111"
    print("-----------------test sign and verify---------------")
    k = randint(1,int(ecdsa_crypt.n,16))
    r, s = ecdsa_crypt.sign(data, k)
    print('sign:%s',r ,s)
    verify = ecdsa_crypt.verify(r, s, data)
